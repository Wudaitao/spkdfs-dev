...
extern void EXPECT_EQ(int a, int b);
    ...
    struct timezone tz1;
    double tsec1;
pthread_t decode_erasures_thread[50];
pthread_t decode_uncoupled_thread[50];
//memset(decode_erasures_thread, 0, sizeof(decode_erasures_thread));
//memset(decode_uncoupled_thread, 0, sizeof(decode_uncoupled_thread));
static int de_count = 0;
static int du_count = 0;
void* decode_erasures(void* arg);
//void* get_coupled_from_uncoupled(void* arg);
struct parameter
{
...
int Clay::encode(const set<int> &want_to_encode,char *fname)
{
    ...
    data_size = fp.tellg();
    payload = (char*)malloc(sizeof(char)*data_size+1);
    ...
    std::ofstream fp2;
    std::ofstream meta;
    char *curdir = (char*)malloc(sizeof(char)*1000);
    getcwd(curdir, 1000);
    char *fname2 = (char*)malloc(sizeof(char)*(strlen(curdir)+10));
    memset(fname2,0,strlen(curdir)+10);
    char *meta_fname = (char*)malloc(sizeof(char)*(strlen(curdir)+10));
    memset(meta_fname,0,strlen(curdir)+10);
    ostringstream out;
...
//    }
    ...
    {
        sprintf(fname2, "%s/Coding/%s_%d.txt", curdir, filename, i+1);
        fp2.open(fname2);
        if(!fp2)
    ...
    ////写入元数据
    sprintf(meta_fname, "%s/Coding/meta.txt", curdir);
    meta.open(meta_fname);
    if(!meta)
        std::cerr << "create file failed. maybe wrong directory." << std::endl;
    out<<blocksize<<endl;
    out<<subchunk_size<<endl;
    out<<data_size<<endl;
    meta<<out.str()<<endl;
    meta.close();
    ////free
...
par-> erased_chunks = erased_chunks;
par->chunks = chunks;
par->sc_size = sc_size;
par->clay = this;
int a = de_count;
...
pthread_create(&decode_erasures_thread[de_count], NULL, decode_erasures, (void*)par);
de_count++;
            }
        }
for(int i = a; i<de_count;i++)
	pthread_join(decode_erasures_thread[a], NULL);
...
par->y = y;
par->z = z;
//pthread_create(&decode_uncoupled_thread[du_count], NULL, get_coupled_from_uncoupled, (void*)par);
//du_count++;
...
//for(int i = a; i<du_count;i++)
	//pthread_join(decode_uncoupled_thread[a], NULL);
    } // iscore, order
//gettimeofday(&t6, &tz1);
...
void* decode_erasures(void* arg)
{
struct parameter *ptr = (struct parameter*)arg;
int z = ptr->z;
Clay *clay = ptr->clay;
set<int> erased_chunks = ptr-> erased_chunks;
map<int, bufferlist>* chunks = ptr-> chunks;
int sc_size  = ptr->sc_size;
    int z_vec[clay->t];
    std::cout << "#decode_erasures#" << std::endl;
    clay->get_plane_vector(z, z_vec);

    for (int x = 0; x < clay->q; x++)
    {
        for (int y = 0; y < clay->t; y++)
        {
            int node_xy = clay->q * y + x;
            int node_sw = clay->q * y + z_vec[y];
            ////未丢失耦合块得出相应的未耦合块
            if (erased_chunks.count(node_xy) == 0)
            {
                if (z_vec[y] < x)
                {
                    ...
                    clay->get_uncoupled_from_coupled(chunks, x, y, z, z_vec, sc_size);
                }
                else if (z_vec[y] == x)    ////x=z_vec[y]对应论文x=zy,这部分顶点是未配对的，C=U
                {
                    ...
                    set_U1(coupled_chunk, &(clay->U_buf)[node_xy], z * sc_size, sc_size);////设置U   C和U的偏移均为z*sc_size
                    free(coupled_chunk);
                }
                else
                {
    ...
    //cout << "end" << endl;
     clay->decode_uncoupled(erased_chunks, z, sc_size);////mds的解码
return NULL;
}

//void* get_coupled_from_uncoupled(void* arg)
void Clay::get_coupled_from_uncoupled(std::map<int, bufferlist>* chunks, int x, int y, int z, int* z_vec, int sc_size)
{
...
//                            std::cout<<"设置第"<<node_xy<<"个U_buf偏移为"<<z*sub_chunksize<<"长度为"<<sub_chunksize<<std::endl;
                        ...
                        else
                        {
                            ...
                            if (z_vec[y] != x)
                            {
                                ...
                                pftsubchunks[i1] = known_subchunks[i1];
                                substr_of(&pftsubchunks[i2],U_buf[node_xy], z*sub_chunksize, sub_chunksize);////恢复U_buf[node_xy]
                                substr_of(&pftsubchunks[i3],temp_buf, 0, sub_chunksize,sub_chunksize,1);////temp_buf
                                scalar_decode_chunks(atoi(pft.profile["k"].c_str()), atoi(pft.profile["m"].c_str()), atoi(pft.profile["w"].c_str()), pft_erasures, known_subchunks, &pftsubchunks, sub_chunksize); ////调用MDS的解码函数
//                                printf("2\n");
//                                std::cout<<"设置第"<<node_xy<<"个U_buf偏移为"<<z*sub_chunksize<<"长度为"<<sub_chunksize<<" "<<std::endl;
                                set_U(pftsubchunks[i2], &U_buf[node_xy], z*sub_chunksize, sub_chunksize, sub_chunk_no*sub_chunksize);////设置U
