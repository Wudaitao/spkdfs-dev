...
////新添加
////k，m，w自己传,chunks是现有的块，decoded保存所有块，损毁的置0
int scalar_encode_chunks(int k, int m, int w, map<int, bufferlist> *decoded,int subsize)
{
    ...
    for (int i =  0; i < k + m; i++)
    {
        if (i < k) ////未丢失的数据块放入相应的data数据块或者coding校验块
        {
            data[i] = c_str((*decoded)[i], subsize + 1);
        }
        else
        {
            coding[i - k] = (char *)malloc(sizeof(char)*subsize);
        }
    }
    jerasure_encode(k, m, w, data, coding, subsize);

    for (int i =  k; i < k+m; i++)
    {
        (*decoded)[i].push_back(coding[i - k]);////校验块赋值
    }

    ////测试
    //printf("%s:%s\n","data",c_str((*decoded)[0], subsize + 1));
    //printf("%s:%s\n","data",c_str((*decoded)[1], subsize + 1));
    //printf("%s:%s\n","coding",c_str((*decoded)[2], subsize + 1));
    //printf("%s:%s\n","coding",c_str((*decoded)[3], subsize + 1));

    return 0;
}
////data是全部数据块，coding是全部校验块，损毁的置0
int jerasure_encode(int k, int m, int w, char **data, char **coding, int blocksize)
{
    int *matrix;
    matrix = reed_sol_vandermonde_coding_matrix(k, m, w);
    //return jerasure_matrix_decode(k, m, w, matrix, 1, erasures, data, coding, blocksize);/////////用这个解码
    jerasure_matrix_encode(k, m, w, matrix, data, coding, blocksize);/////////用这个解码
    return 0;
}
...
int scalar_decode_chunks(int k, int m, int w, const set<int> &want_to_read, const map<int, bufferlist> &chunks, map<int, bufferlist> *decoded,int subsize)
{
    ...
    for (int i =  0; i < k + m; i++)
    {
        ...
        {
            erasures[erasures_count] = i;////将丢失的块放入erasures数组
            erasures_count++;
        }
        else
        {
            if (i < k)  ////未丢失的数据块放入相应的data数据块或者coding校验块
            {
                data[i] = (char *)malloc(sizeof(char)*subsize);
//                for(int x = 0; x<subsize; x++)
//                {
//                    //printf("%c",(*decoded)[i].front()[x]);
//                    data[i][x]= (*decoded)[i].front()[x];////覆盖
//                }
                memcpy(data[i],(*decoded)[i].front(),subsize);
            }
            else
            {
                coding[i - k] = (char *)malloc(sizeof(char)*subsize);
...
//                {
//                    //printf("%c",(*decoded)[i].front()[x]);
//                    coding[i - k][x] = (*decoded)[i].front()[x];////覆盖
//                }
                memcpy(coding[i - k],(*decoded)[i].front(),subsize);
            }
        }
        //printf("\n");
    }
    ...
    for (int i = 0; i < erasures_count; i++)
    {
        if (erasures[i] < k)
        {
            data[erasures[i]] = (char *)malloc(sizeof(char)*subsize);
            memset(data[erasures[i]],0,subsize);
        }
        else
        {
            coding[erasures[i]-k] = (char *)malloc(sizeof(char)*subsize);
            memset(coding[erasures[i]-k],0,subsize);
        }
    }

    erasures[erasures_count] = -1;
    assert(erasures_count > 0);////为true即有块丢失继续往下执行jerasure_decode
    jerasure_decode(k, m, w, erasures, data, coding, subsize);
...
//            for(int x = 0; x<subsize; x++)
//            {
//                //printf("%c",data[erasures[i]][x]);
//                (*decoded)[erasures[i]].front()[x] = data[erasures[i]][x];
//            }
            memcpy((*decoded)[erasures[i]].front(),data[erasures[i]],subsize);
            //(*decoded)[erasures[i]].push_back(data[erasures[i]]);
        }
        else
        {
            //printf("\ncoding:");
//            for(int x = 0; x<subsize; x++)
//            {
//                //printf("%c",coding[erasures[i]-k][x]);
//                (*decoded)[erasures[i]].front()[x] = coding[erasures[i]-k][x];
//            }
            memcpy((*decoded)[erasures[i]].front(),coding[erasures[i]-k],subsize);
        }
    }
    ////释放内存
    for (int i =  0; i < k + m; i++)
    ...
    ////测试
//    printf("\n%s:%s\n","data",c_str((*decoded)[0], subsize));
//    printf("%s:%s\n","data",c_str((*decoded)[1], subsize));
//    printf("%s:%s\n","coding",c_str((*decoded)[2], subsize));
//    printf("%s:%s\n","coding",c_str((*decoded)[3], subsize));


    //printf("scalar_decode_chunks complete\n");
    return 0;
}
////data是全部数据块，coding是全部校验块，损毁的置0
int jerasure_decode(int k, int m, int w, int *erasures, char **data, char **coding, int blocksize)
{
    ...
    matrix = reed_sol_vandermonde_coding_matrix(k, m, w);
    //jerasure_matrix_decode(k, m, w, matrix, 1, erasures, data, coding, blocksize);/////////用这个解码
...
//    jerasure_schedule_decode_lazy(k, m, w, bitmatrix, erasures, data, coding, blocksize, 1024, 1);
