...
static int create_storage_dirs(cycle_t *cycle)
{
    ...
    while (head != entry)
    {
	    ...
	    {
	        if (mkdir(sd->current, 
				S_IRWXU | S_IRWXG | S_IXOTH | S_IROTH) != NGX_OK) //S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH
	        {
	            dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno, 
...
int setup_ns_storage(dfs_thread_t *thread)
{
    ...
    while (head != entry)
    {
        auto *sd = queue_data(entry, storage_dir_t, me);
...
static int check_namespace(char *path, int64_t namespaceID)
{
    ...
    if (access(path, F_OK) != NGX_OK)
	{
	    if (mkdir(path, S_IRWXU | S_IRWXG | S_IXOTH | S_IROTH) != NGX_OK)
	    {
	        dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno, 
				"mkdir %s err", path);
		...
		if (mkdir(bbwDir, S_IRWXU | S_IRWXG | S_IXOTH | S_IROTH) != NGX_OK)
	    {
	        dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno, 
				"mkdir %s err", bbwDir);
		...
		if (mkdir(curDir, S_IRWXU | S_IRWXG | S_IXOTH | S_IROTH) != NGX_OK)
	    {
	        dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno, 
				"mkdir %s err", curDir);
...
static int create_storage_subdirs(char *path)
{
	...
	{	
		...
		{
            if (mkdir(subDir, S_IRWXU | S_IRWXG | S_IXOTH | S_IROTH) != NGX_OK)
	        {
	            dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno, 
					"mkdir %s err", subDir);
			...
			for (int j = 0; j < SUBDIR_LEN; j++) 
			{
				...
				if (mkdir(ssubDir, S_IRWXU | S_IRWXG | S_IXOTH | S_IROTH)
					!= NGX_OK)
	            {
	                dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno, 
						"mkdir %s err", ssubDir);
