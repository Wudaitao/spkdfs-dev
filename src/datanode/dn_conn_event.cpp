...
typedef void (*conn_init_func) (conn_t *c);

//
static void rev_handler(event_t *ev, conn_init_func func);
// beginning, this is for cli
static void cli_listen_rev_handler(event_t *ev);

// todo: this is prepare for dn and nn
...
int conn_listening_init(cycle_t *cycle)
{
    ...
    listening_for_open = (server_bind_t *)sconf->listen_open_port.elts; //cli server_bind_t addr prot
	...
	for (i = 0; i < sconf->bind_for_cli.nelts; i++) 
	{
        ...
        ls = conn_listening_add(&cycle->listening_for_cli, cycle->pool,
            ...
            bind_for_cli[i].port, cli_listen_rev_handler,
...
//        dbg(cycle->listening_ip);

    }
//    dbg(dfs_cycle->listening_ip);
    //
    cycle->listening_for_open.elts = pool_calloc(cycle->pool,
                                                sizeof(listening_t) * sconf->listen_open_port.nelts);
    if (!cycle->listening_for_open.elts)
    {
        dfs_log_error(cycle->error_log, DFS_LOG_FATAL, 0,
                      "no space to alloc listening_for_cli pool");
    ...
    cycle->listening_for_open.nelts = 0;
    cycle->listening_for_open.size = sizeof(listening_t);
    cycle->listening_for_open.nalloc = sconf->listen_open_port.nelts;
    cycle->listening_for_open.pool = cycle->pool;

    for (i = 0; i < sconf->listen_open_port.nelts; i++)
    {
        // add listening to array
        // init listening
        ls = conn_listening_add(&cycle->listening_for_open, cycle->pool,
                                cycle->error_log, inet_addr((char *)listening_for_open[i].addr.data),
                                listening_for_open[i].port, listen_rev_handler,
                                ((conf_server_t*)cycle->sconf)->recv_buff_len,
                                ((conf_server_t*)cycle->sconf)->recv_buff_len);
        ...
        strcpy(cycle->listening_open_ip, (const char *)listening_for_open[i].addr.data);
        cycle->listening_open_port = listening_for_open[i].port; //
    ...
    if (conn_listening_open(&cycle->listening_for_open, cycle->error_log)
        != NGX_OK)
    {
        return NGX_ERROR;
    }
...
static void rev_handler(event_t *ev, conn_init_func func)
{
         ...
         i++)
    {
        ...
        nc->sockaddr = (struct sockaddr *)pool_calloc(nc->pool, socklen);
        if (!nc->sockaddr)
        {
            dfs_log_error(dfs_cycle->error_log, DFS_LOG_ALERT, 0,
...
static void cli_listen_rev_handler(event_t *ev)
{
    rev_handler(ev,dn_conn_init);
}

// add: and for diff: just change dn_conn_init to nn_conn_init_
// 处理函数
// accept handler
static void listen_rev_handler(event_t *ev)
{
    rev_handler(ev, nn_conn_init_);
}
