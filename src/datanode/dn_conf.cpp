...
static conf_option_t conf_server_option[] = 
{
    ...
    { string_make("max_group_nodes"), conf_parse_int,
      OPE_EQUAL, offsetof(conf_server_t, max_group_nodes) },

    { string_make("bind_for_cli"), conf_parse_bind,
        OPE_EQUAL, offsetof(conf_server_t, bind_for_cli) },

    { string_make("ns_srv_port"), conf_parse_int,
            OPE_EQUAL, offsetof(conf_server_t, ns_srv_port) },

	{ string_make("ns_srv"), conf_parse_string,
        OPE_EQUAL, offsetof(conf_server_t, ns_srv) },

    { string_make("listen_open_port"), conf_parse_bind,
            OPE_EQUAL, offsetof(conf_server_t, listen_open_port) },
    ...
    { string_make("persistent_data"), conf_parse_string,
            OPE_EQUAL, offsetof(conf_server_t, persistent_data) },
    ...
    { string_make("heartbeat_timeout"), conf_parse_int,
            OPE_EQUAL, offsetof(conf_server_t, heartbeat_timeout) },
...
static void *conf_server_init(pool_t *pool)
{
    ...
    if (array_init(&sconf->listen_open_port, pool, CONF_SERVER_BIND_N,
                   sizeof(server_bind_t)) != NGX_OK)
    {
        return nullptr;
    }
...
static int conf_server_make_default(void *var)
{
    ...
    std::string pid_file_= expand_user("~") + PID_FILE;
    set_def_string(&sconf->pid_file,            pid_file_.c_str());
...
void conf_set_my_paxos(char *addrString, int len) {
    auto * sconf = static_cast<conf_server_t *>(dfs_cycle->sconf);
    if(sconf->my_paxos.data!= nullptr){
        free(sconf->my_paxos.data);
    }
    sconf->my_paxos.data = reinterpret_cast<uchar_t *>(strdup(addrString));
    sconf->my_paxos.len = len;
}
