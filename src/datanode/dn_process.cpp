	                              ...
	                              void *data, char *name, int slot)
{
    ...
    pid = fork();
    // 这里是第三个进程了
    printf("ngx_worker_process_cycle pid:%d\n",pid);
    switch (pid) 
	{
...
void process_master_cycle(cycle_t *cycle, int argc, char **argv)
{
    ...
    // cli_listen_rev_handler 处理 listening 事件
    ...
    //
    char ownIp[INET_ADDRSTRLEN] = {0};

    int ret = get_local_ip(ownIp);
    if (ret == NGX_ERROR) {
        fprintf(stderr, "get_local_ip fail\n");
        return;
    }
    dn_group->setOwn(ownIp, dfs_cycle->listening_paxos_port); // default port 0
    strcpy(dfs_cycle->listening_ip, ownIp);
    dbg(dn_group->getOwn().getNodeIp());
    //
...
int process_get_pid(cycle_t *cycle)
{
    ...
    if (fd < 0) 
	{
        dfs_log_error(cycle->error_log,DFS_LOG_WARN, errno,
            "process_get_pid() process_write_pid_file failed!");
...
int process_get_nnpid()
{
    ...
    snprintf(sTmp, sizeof(sTmp), "%s/data/namenode/pid/namenode.pid", prefix.c_str());

    pid_file = (u_char *)sTmp;
