...
static int parse_cmdline(int argc, char *const *argv) {
    char buf[255] = {0};
    args::ArgumentParser parser("datanode usage.", "end");
    args::HelpFlag help(parser, "help", "Display this help menu", {'h', "help"});
    args::ValueFlag<std::string> config(parser, "config", " Configure file", {'c'});
    args::ValueFlag<std::string> iprange(parser, "iprange", " Scan IP ranges, eg:127.0.0.1-127.0.0.128 ", {'r'});
    args::ValueFlag<int> ipnum(parser, "ipnum", " the definite device num ", {'n'});
    args::Flag version(parser, "version", "version", {'v', 'V'});
    args::Flag stop(parser, "stop", " stop datanode server", {'q'});
    ...
    if(!iprange){
        printf("iprange and ipnum needed!\n");
        return NGX_ERROR;
    }
    if(iprange){
...
int main(int argc, char **argv) {
    ...
    DEFAULT_CONF_FILE =  prefix + "/etc/datanode.conf";
    ...
    // init group
    dn_group = DfsGroup::getInstance("dn_group");
    old_dn_group =new DfsGroup("old_dn_group");
    old_dn_group->load_persistent_grouplist();
    //
    //
