...
static int update_rbi_finish(const char *key , report_blk_info_t *rbi);
...
int nn_open(task_t *task) {
    ...
    if(!blk){
        ...
        task->ret = FILE_BLK_NOT_EXIST;

        return write_back(node);
    }
    resp_info.blk_id = blk->id;
    resp_info.blk_sz = blk->size;
    resp_info.t_blk_sz = fin.t_blk_sz;
    resp_info.t_sub_sz = fin.t_sub_sz;
    //resp_info.t_help_sz = fin.t_help_sz;
    ...
    if (blk->dn_num == 1){
        ...
        resp_info.dn_num = 2;
        strcpy(resp_info.dn_ips[0], blk->dn_ip);
        strcpy(resp_info.dn_ips[1], blk->dn_ip1);
        strcpy(resp_info.dn_ips[2], "");
    } else if(blk->dn_num == 3){
        resp_info.dn_num = 3;
        strcpy(resp_info.dn_ips[0], blk->dn_ip);
        strcpy(resp_info.dn_ips[1], blk->dn_ip1);
        strcpy(resp_info.dn_ips[2], blk->dn_ip2);
    }
...
int update_fi_cache_mgmt(const uint64_t llInstanceID,
    ...
    switch (optype) {
        ...
        case NN_CREATE:
            ...
            /*fin.t_blk_sz = lopr.mutable_cre()->t_blk_sz();
            fin.t_sub_sz = lopr.mutable_cre()->t_sub_sz();
            fin.t_help_sz = lopr.mutable_cre()->t_help_sz();  */
...
//            dfs_log_error(dfs_cycle->error_log,DFS)
            report_blk_info_t rbi;
            bzero(&rbi, sizeof(report_blk_info_t));
            key = lopr.mutable_luf()->key();
            strcpy(rbi.dn_ip,lopr.mutable_luf()->dn_ip().c_str());
            rbi.blk_sz = lopr.mutable_luf()->blk_size();
            rbi.blk_id = lopr.mutable_luf()->blk_id();
            update_rbi_finish(key.c_str(),&rbi);
            break;
        }
...
*/
    ...
    // end
    auto *fis = (fi_store_t *)mem_get0(g_fcm->mem_mgmt.free_mblks);
...
int load_image() {
    ...
    auto *conf = (conf_server_t *) dfs_cycle->sconf;
...
static int update_fi_create(fi_inode_t *fin, uint64_t blk_id, void *data) {
    dbg("update_fi_create");
    char tmppath[256] = {0};
    keyDecode(reinterpret_cast<uchar_t *>(fin->key), reinterpret_cast<uchar_t *>(tmppath));
    dbg(tmppath);

    auto *thread = (dfs_thread_t *) data;
    ...
    if (fin->blk_seq < 1 || fin->blk_seq > fin->total_blk || fin->blk_seq > BLK_LIMIT) {
        dfs_log_error(dfs_cycle->error_log, DFS_LOG_ERROR, errno,
                      "read blk_seq %d err", fin->blk_seq);
        return NGX_ERROR;
    }
...
static int update_fi_close(fi_inode_t *fin) {
    ...
    // no fis
    if(!fis){
        dfs_log_error(dfs_cycle->error_log,DFS_LOG_ERROR,0,"update_fi_close fis error,key:%s ",fin->key);
    }
    if (fis->thread != nullptr) {
    ...
    auto *fparent = (fi_store_t *) dfs_hashtable_lookup(g_fcm->fi_htable,
                                                              (void *) pKey, string_strlen(pKey));
    if(!fparent){
        dfs_log_error(dfs_cycle->error_log,DFS_LOG_ERROR,0,"update_fi_close fparent error,"
                                                           "path : %s, pkey:%s ",path, pKey);
        return NGX_ERROR;
    }
...
static int update_rbi_finish(const char *key , report_blk_info_t *rbi){
    ...
    return nn_update_rbi_finish(key,rbi);
}
