...
static int log_upload_finish(task_t *task);
...
{
    int optype = task->cmd;
    dbg(g_editlog->GetMaster(task->key).GetPort());
	...
	case NN_GET_ADDITIONAL_BLK: // nouse
		log_get_additional_blk(task);
		break;

    case DN_RECV_BLK_REPORT:
        log_upload_finish(task);
        break;
	...
	fi_inode_t *finodes[], int num)
{
	...
	for (int i = 0; i < num; i++) 
	{
		...
		if (check_permission(task, (fi_inode_t_*)finodes[i], EXECUTE, err) != NGX_OK)
		{
		    dfs_log_error(dfs_cycle->error_log, DFS_LOG_ALERT, 0, 
	...
	short access, fi_inode_t *finode)
{
	...
	if (check_permission(task, (fi_inode_t_*)finode, access, err) != NGX_OK)
	{
        dfs_log_error(dfs_cycle->error_log, DFS_LOG_ALERT, 0, 
			"check_permission err: %s, path: %s", err, path);
...
static int log_mkdir(task_t *task)
{
    ...
    if (!g_editlog->IsIMMaster(task->key)) // 不是master 节点 就重定向到master节点
	{
        ...
        if(g_editlog->GetMaster(task->key).GetPort() == 0){
            dfs_log_error(dfs_cycle->error_log,DFS_LOG_ERROR,0,"get namenode master failed");
            task->ret = NO_MASTER;
            return write_back(node);
        }
        task->data_len = INET_ADDRSTRLEN + 1 ;
        task->data = malloc(task->data_len);
        memcpy(task->data,g_editlog->GetMaster(task->key).GetIP().c_str(),task->data_len);
        return write_back(node);
	}
...
static int log_create(task_t *task)
{
    ...
    dbg(blk_info.blk_rep);
	//  数据布局：从 g_dn_q 中选择一个存储节点 datanode
...
//    dbg(resp_info.dn_ips[0]);
    ...
    /*lopr.mutable_cre()->set_t_blk_sz(blk_info.t_blk_sz);
    lopr.mutable_cre()->set_t_sub_sz(blk_info.t_sub_sz);
    lopr.mutable_cre()->set_t_help_sz()(blk_info.t_help_sz());*/
	string sPaxosValue;
	lopr.SerializeToString(&sPaxosValue);
...
static int log_upload_finish(task_t *task){
    ...
    LogOperator lopr;
    lopr.set_optype(task->cmd);
    lopr.mutable_luf()->set_key((const char *)task->key);
    lopr.mutable_luf()->set_blk_id(rbi.blk_id);
    lopr.mutable_luf()->set_blk_size(rbi.blk_sz);
    lopr.mutable_luf()->set_dn_ip(rbi.dn_ip);
...
static int log_close(task_t *task)
{
	...
	if (!g_editlog->IsIMMaster(task->key))
	{
        task->ret = MASTER_REDIRECT; //10
		task->master_nodeid = g_editlog->GetMaster(task->key).GetNodeID();
        if(g_editlog->GetMaster(task->key).GetPort() == 0){
            dfs_log_error(dfs_cycle->error_log,DFS_LOG_ERROR,0,"get namenode master failed");
            task->ret = NO_MASTER;// 101
            return write_back(node);
        }
        task->data_len = INET_ADDRSTRLEN + 1 ;
        task->data = malloc(task->data_len);
