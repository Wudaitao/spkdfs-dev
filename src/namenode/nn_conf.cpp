...
static conf_option_t conf_server_option[] = 
{
    ...
    { string_make("enableMaster"), conf_parse_int,
            OPE_EQUAL, offsetof(conf_server_t, enableMaster) },
...
void nn_conf_set_ot_paxos(cycle_t *cycle) {
    auto * sconf = static_cast<conf_server_t *>(dfs_cycle->sconf);
    if(sconf->ot_paxos.data!= nullptr){
        free(sconf->ot_paxos.data);
        sconf->ot_paxos.data = nullptr;
    }
    sconf->ot_paxos.data = (uchar_t *) cycle->ot_paxos_string.c_str();
    sconf->ot_paxos.len = cycle->ot_paxos_string.length();
