...
static int cfs_faio_open(uchar_t *path, int flags, log_t *log)
{
    ...
    fd = dfs_sys_open(path, flags, S_IRWXU); // open 0400 -> S_IRWXU
    if (fd < 0) 
	{
        return fd;
    }
...
//        printf("cfs_faio_io_send_file rc:%d connfd:%d filefd:%d offset:%ld need:%d\n ",rc,sf_chain_task->conn_fd, sf_chain_task->store_fd,
//               &file_task->offset, file_task->need);
//        dbg(errno); //11
        if (rc == NGX_ERROR)
		{
//            dfs_log_error(dfs_cycle->error_log,DFS_LOG_ERROR,errno,"errno:%d\n",errno);
            if (errno == DFS_EAGAIN)  //11
			{
                file_task->faio_ret = DFS_EAGAIN;
				
                return NGX_OK;
            } 
			else if (errno == DFS_EINTR) // 4
			{
                continue;
            }
