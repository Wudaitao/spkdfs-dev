...
static long do_recvfile_splice(int fromfd, int tofd,
...
int dfscli_get(char *src, char *dst, const int *blk_num)
{
	...
	rw_ctx = (rw_context_t *)pool_calloc(dfscli_cycle->pool, sizeof(rw_context_t));
	if (!rw_ctx) 
	{
...
static int dfs_open(rw_context_t *rw_ctx)
{
	...
	if (!pNext) 
	{
	    dfscli_log(DFS_LOG_WARN, "pNext pool_alloc err,recv pLen: %d", pLen);
    ...
    if (in_t.ret != NGX_OK)
	{
		...
		else if (in_t.ret == FILE_BLK_NOT_EXIST){
            dfscli_log(DFS_LOG_WARN, "open err, FILE_BLK_NOT_EXIST");
        }
		else 
		{
	...
	else if (nullptr != in_t.data && in_t.data_len > 0)
	{
		...
		blocksize = resp_info.t_blk_sz;
		subchunk_size = resp_info.t_sub_sz;
		helper_blocksize = resp_info.t_help_sz;
		//
...
static int dfs_read_blk(rw_context_t *rw_ctx)
{
	...
	while(sockfd < 0){
        dn_index += 1;
	    if(dn_index==rw_ctx->dn_num){
            close(datafd);
            unlink(rw_ctx->dst);
            return NGX_ERROR;
	    }
        sockfd = dfs_connect(rw_ctx->dn_ips[dn_index], DN_PORT);
	}
//	if (sockfd < 0)
//	{
//	    if(rw_ctx->dn_num == 2){
//	        dn_index += 1;
//            sockfd = dfs_connect(rw_ctx->dn_ips[dn_index], DN_PORT);
//	    }else if(sockfd < 0 && rw_ctx->dn_num == 3){
//            dn_index += 1;
//            sockfd = dfs_connect(rw_ctx->dn_ips[dn_index], DN_PORT);
//        }else{
//            close(datafd);
	...
	loff_t *offset, size_t count)
{
    ...
    while (rc > 0) 
	{
         ...
         */
        nread = splice(fromfd, nullptr, pipefd[1], nullptr, MIN(rc, 16384),
            SPLICE_F_MOVE);
        if (nread < 0) 
		{
