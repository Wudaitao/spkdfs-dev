...
static void *cut_write_blk_start(void *arg);

static long write_blk_to_dn(long fsize, int out_fd, int in_fd);
...
int dfscli_put(char *src, char *dst, int blk_seq, int total_blk) {
    ...
    rw_ctx = (rw_context_t *) pool_calloc(dfscli_cycle->pool,
                                         sizeof(rw_context_t));
    if (!rw_ctx) {
        dfscli_log(DFS_LOG_WARN, "dfscli_put, rw_ctx pool_alloc err");
    ...
    memory_zero(rw_ctx,sizeof(rw_context_t));
    return NGX_OK;
}
...
redo:
    ...
    if (sockfd < 0) {
        ...
        sockfd = dfs_connect(dfscli_cycle->namenode_addr, nn_addr[0].port);
        if (sockfd < 0) return NGX_ERROR;
    }
    ...
    // end
    /*blk_info.t_blk_sz = blocksize;
    blk_info.t_sub_sz = subchunk_size;
    blk_info.t_help_sz = helper_blocksize;*/
    ...
    pNext = (char *) pool_alloc(dfscli_cycle->pool, pLen);
    if (!pNext) {
...
redo:
    ...
    if (in_t.ret != NGX_OK) {
        if (in_t.ret == MASTER_REDIRECT){
            close(rw_ctx->nn_fd);
            memcpy(dfscli_cycle->namenode_addr, in_t.data,in_t.data_len);
            int sockfd = dfs_connect(dfscli_cycle->namenode_addr, NAME_NODE_PORT);
            if(sockfd < 0){
                dfscli_log(DFS_LOG_WARN, "nn close error : can not connect to master now.");
            }
            rw_ctx->nn_fd = sockfd;
            goto redo;
        } else if(in_t.ret == NO_MASTER){
...
static void *write_blk_start(void *arg) {
    auto *p = (parameter *)arg;
    int dn_index = p->dn_index;
    int res = -1;
    rw_context_t *rw_ctx = p->rw_ctx;
    auto *sconf = (conf_server_t *) dfscli_cycle->sconf;
    ...
    dfscli_log(DFS_LOG_INFO, "put file %s to remote %s,  %s succesfully.",
               rw_ctx->src,rw_ctx->dn_ips[dn_index], rw_ctx->dst);
...
static void *cut_write_blk_start(void *arg) {
    ...
    int res = -1;
    conf_server_t *sconf = (conf_server_t *) dfscli_cycle->sconf;
...
// transfer file between two sockfd, data fd
static long write_blk_to_dn(long fsize, int out_fd, int in_fd) // sockfd, data fd
{
    loff_t off = 0; //  change loff_t
    long send_sz = 0;
    long done = 0;
...
static int dfs_get_additional_blk(rw_context_t *rw_ctx) {
    ...
    char *pNext = (char *) pool_alloc(dfscli_cycle->pool, pLen);
    if (!pNext) {
        dfscli_log(DFS_LOG_WARN, "pool_alloc err, pLen: %d", pLen);
